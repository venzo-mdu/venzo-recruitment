  ...
  try {
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        status: 'PENDING',
        ai_summary: null,
        job_post_id: candidateData.jobPostId || null,
      }])
      .select()
      .single();
    ...
    if (error) {
      ...
      if (error.code === '23505' && error.message.includes('email')) {
        userMessage = 'This email address has already been used to apply for this job. Please use a different email or contact HR if you need to update your application.';
      }
...
export const getCandidates = async (filters = {}) => {
  try {
    ...
    // Apply filters
    if (filters.jobPostIds && filters.jobPostIds.length > 0) {
      query = query.in('job_post_id', filters.jobPostIds);
    } else if (filters.jobPostId) {
